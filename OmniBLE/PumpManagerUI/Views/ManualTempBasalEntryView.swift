...
struct ManualTempBasalEntryView: View {
    ...
    var missingConfigAlert: SwiftUI.Alert {
        return SwiftUI.Alert(
            ...
            message: Text(LocalizedString("该PumpManager尚未配置为最大基础速率，因为在手动临时基础率是一个功能之前，它已被添加。请设置新的最大基础速率。", comment: "Alert format string for missing temp basal configuration."))
        )
    }
