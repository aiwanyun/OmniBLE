...
struct ManualTempBasalEntryView: View {
    ...
    var missingConfigAlert: SwiftUI.Alert {
        return SwiftUI.Alert(
            ...
            message: Text(LocalizedString("该PumpManager尚未配置为最大基础速率，因为在手动温度基础是一个功能之前，它已被添加。请转到治疗设置 - >交货限制并设定新的最大基础速度。", comment: "Alert format string for missing temp basal configuration."))
        )
    }
